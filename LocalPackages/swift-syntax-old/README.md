...
The swift-syntax package is a set of libraries that work on a source-accurate tree representation of Swift source code, called the SwiftSyntaxOld tree. The SwiftSyntaxOld tree forms the backbone of Swift’s macro system – the macro expansion nodes are represented as SwiftSyntaxOld nodes and a macro generates a SwiftSyntaxOld tree to be inserted into the source file.
...
A great way to interactively explore the SwiftSyntaxOld tree of a source file is https://swift-ast-explorer.com, developed by [@kishikawakatsumi](https://github.com/kishikawakatsumi).

A set of example usages of swift-syntax can be found in [Examples](Examples).

## Releases

Releases of SwiftSyntaxOld are aligned with corresponding language and tooling releases, for example the major version 509 of swift-syntax is aligned with Swift 5.9. 
...
If you should hit any issues while using SwiftSyntaxOld, we appreciate bug reports on [GitHub Issue](https://github.com/swiftlang/swift-syntax/issues).

## Contributing

Start contributing to SwiftSyntaxOld see [this guide](CONTRIBUTING.md) for more information.

## Bazel

SwiftSyntaxOld provides an experimental [Bazel](https://bazel.build) build configuration, maintained by Keith Smiley. 
...
SwiftSyntaxOld to always build with optimizations enabled. This may help
