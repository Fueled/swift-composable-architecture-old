  ...
  fileprivate struct AddCompletionHandler: PeerMacro {
    static func expansion(
      ...
      guard let funcDecl = declaration.as(FunctionDeclSyntax.self) else {
        throw SwiftSyntaxMacrosOld.MacroExpansionErrorMessage("@addCompletionHandler only works on functions")
      }
      ...
      if funcDecl.signature.effectSpecifiers?.asyncSpecifier == nil {
        ...
        let diag = Diagnostic(
          ...
          message: SwiftSyntaxMacrosOld.MacroExpansionErrorMessage(
            "can only add a completion-handler variant to an 'async' function"
          ),
          fixIts: [
            .replaceChild(
              message: SwiftSyntaxMacrosOld.MacroExpansionFixItMessage("add 'async'"),
              parent: funcDecl,
