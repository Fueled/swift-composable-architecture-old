...
#if swift(>=6)
public import SwiftBasicFormatOld
internal import SwiftDiagnosticsOld
@_spi(RawSyntax) @_spi(Testing) internal import SwiftParserOld
@_spi(RawSyntax) public import SwiftSyntaxOld
#else
import SwiftBasicFormatOld
import SwiftDiagnosticsOld
@_spi(RawSyntax) @_spi(Testing) import SwiftParserOld
@_spi(RawSyntax) import SwiftSyntaxOld
#endif
...
extension SyntaxStringInterpolation: StringInterpolationProtocol {
  ...
  ///
  /// - SeeAlso: ``SwiftSyntaxOld/ExprSyntax/init(literal:)``
  public mutating func appendInterpolation(
...
/// A string interpolation error based on a ``SwiftDiagnosticsOld/Diagnostic``.
...
/// `ExpressibleByLiteralSyntax` but ``SwiftSyntaxOld/StringLiteralExprSyntax`` is not.
///
/// This protocol is usually not used directly. Instead, conforming types can
/// be turned into syntax trees using ``SwiftSyntaxOld/ExprSyntax/init(literal:)``:
///
...
struct UnexpectedTrivia: DiagnosticMessage {
  ...
  let diagnosticID = MessageID(domain: "SwiftSyntaxBuilderOld", id: "UnexpectedTrivia")
