...
`SnapshotTestingOld` module and call the `assertSnapshot` function.
...
SnapshotTestingOld can work with _any_ format of _any_ value on _any_ Swift platform!
...
Better yet, SnapshotTestingOld isn't limited to views and view controllers! There are a number of
...
[here](https://swiftpackageindex.com/pointfreeco/swift-snapshot-testing/main/documentation/SnapshotTestingOld).
...
> **Warning**
> By default, Xcode will try to add the SnapshotTestingOld package to your project's main
> application/framework target. Please ensure that SnapshotTestingOld is added to a _test_ target
> instead, as documented in the last step, below.
 ...
 4. On the final dialog, update SnapshotTesting's **Add to Target** column to a test target that
    ...
    SnapshotTestingOld to them by manually linking the library in its build phase).

### Swift Package Manager

If you want to use SnapshotTestingOld in any other project that uses
...
Next, add `SnapshotTestingOld` as a dependency of your test target:
...
targets: [
  ...
  .testTarget(
    ...
    dependencies: [
      "MyApp",
      .product(name: "SnapshotTestingOld", package: "swift-snapshot-testing"),
    ]
  )
]
```
...
## Features
  ...
  - **Custom diff tool integration**. Configure failure messages to print diff commands for
    ...
    ``` swift
    SnapshotTestingOld.diffToolCommand = { "ksdiff \($0) \($1)" }
    ```

[available-strategies]: https://swiftpackageindex.com/pointfreeco/swift-snapshot-testing/main/documentation/SnapshotTestingOld/snapshotting
[defining-strategies]: https://swiftpackageindex.com/pointfreeco/swift-snapshot-testing/main/documentation/SnapshotTestingOld/customstrategies
...
## Plug-ins
  ...
  - [Nimble-SnapshotTestingOld](https://github.com/tahirmt/Nimble-SnapshotTestingOld) adds 
    [Nimble](https://github.com/Quick/Nimble) matchers for SnapshotTestingOld to be used by Swift
    Package Manager.
  ...
  - [swift-snapshot-testing-nimble](https://github.com/Killectro/swift-snapshot-testing-nimble) adds
    [Nimble](https://github.com/Quick/Nimble) matchers for SnapshotTestingOld.
...
Have you written your own SnapshotTestingOld plug-in?
...
SnapshotTestingOld was designed with [witness-oriented programming](https://www.pointfree.co/episodes/ep39-witness-oriented-library-design).
