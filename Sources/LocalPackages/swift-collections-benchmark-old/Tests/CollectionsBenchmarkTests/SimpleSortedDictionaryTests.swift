...
import XCTest
@testable import CollectionsBenchmarkOld

final class SimpleSortedDictionaryTests: XCTestCase {
  func testInit() {
    let empty = _SimpleSortedDictionary<Int, Int>()
    XCTAssertTrue(empty.isEmpty)

    do {
      let reference = (0 ..< 10).map { ($0, 2 * $0) }
      let d = _SimpleSortedDictionary<Int, Int>(uniqueKeysWithValues: reference)
      d._checkInvariants()
      _expectEqual(d, reference)
    }

    do {
      let reference = (0 ..< 10).map { ($0, 1) }
      let d = _SimpleSortedDictionary<Int, Int>(
        reference + reference,
        uniquingKeysWith: { $0 + $1 })
      d._checkInvariants()
      _expectEqual(d, (0 ..< 10).map { ($0, 2) })
    }

    let literal: _SimpleSortedDictionary = [
      1: 2,
      2: 4,
      3: 6
    ]
  ...
  func testSubscript_Mutations() {
    var d: _SimpleSortedDictionary<Int, Int> = [:]
    for i in 0 ..< 10 {
      d[i] = 1
    }
    for i in 0 ..< 20 {
