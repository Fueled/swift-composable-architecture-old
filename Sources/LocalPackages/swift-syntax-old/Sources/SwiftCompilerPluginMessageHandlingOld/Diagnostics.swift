...
extension MacroExpansionError: DiagnosticMessage {
  ...
  var diagnosticID: SwiftDiagnosticsOld.MessageID {
    .init(domain: "SwiftCompilerPluginOld", id: "\(type(of: self)).\(self)")
  }
  var severity: SwiftDiagnosticsOld.DiagnosticSeverity {
    .error
  }
}
...
extension PluginMessage.Diagnostic.Severity {
  init(from syntaxDiagSeverity: SwiftDiagnosticsOld.DiagnosticSeverity) {
...
extension PluginMessage.Diagnostic {
  init(from syntaxDiag: SwiftDiagnosticsOld.Diagnostic, in sourceManager: SourceManager) {
