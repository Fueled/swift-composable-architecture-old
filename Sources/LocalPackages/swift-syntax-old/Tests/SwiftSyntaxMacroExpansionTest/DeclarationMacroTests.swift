...
import SwiftSyntaxMacrosTestSupportOld
import XCTest
    ...
    struct ErrorMacro: DeclarationMacro {
      static func expansion(
        ...
        guard let firstElement = node.arguments.first,
          ...
          throw SwiftSyntaxMacrosOld.MacroExpansionErrorMessage("#error macro requires a string literal")
        }

        context.diagnose(
          Diagnostic(
            node: Syntax(node),
            message: SwiftSyntaxMacrosOld.MacroExpansionErrorMessage(messageString.content.description)
          )
        )
    ...
    struct DefineBitwidthNumberedStructsMacro: DeclarationMacro {
      static func expansion(
        ...
        guard let stringLiteral = node.arguments.first?.expression.as(StringLiteralExprSyntax.self),
          ...
          throw SwiftSyntaxMacrosOld.MacroExpansionErrorMessage(
