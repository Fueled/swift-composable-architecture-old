...
import SwiftIfConfigOld
import SwiftParserOld
import SwiftSyntaxOld
@_spi(XCTestFailureLocation) @_spi(Testing) import SwiftSyntaxMacrosGenericTestSupportOld
import XCTest
import _SwiftSyntaxGenericTestSupportOld
...
public class EvaluateTests: XCTestCase {
  ...
  func testCanImport() throws {
    assertIfConfig("canImport(SwiftSyntaxOld)", .active)
    assertIfConfig("canImport(SwiftSyntaxOld.Sub)", .active)
    assertIfConfig("canImport(SwiftParserOld)", .inactive)
    assertIfConfig("canImport(SwiftSyntaxOld, _version: 5.9)", .active)
    assertIfConfig("canImport(SwiftSyntaxOld, _version: 5.10)", .inactive)
    assertIfConfig(#"canImport(SwiftSyntaxOld, _version: "5.9")"#, .active)
    assertIfConfig("canImport(SwiftSyntaxOld, _underlyingVersion: 5009)", .active)
    assertIfConfig("canImport(SwiftSyntaxOld, _underlyingVersion: 5009.10)", .inactive)
    assertIfConfig(
      "canImport(SwiftSyntaxOld, _underlyingVersion: 5009.10.5.4.2.3.5)",
      .inactive,
      diagnostics: [
        DiagnosticSpec(
    ...
    assertIfConfig(
      "canImport(SwiftSyntaxOld, _version: 20A301)",
      .unparsed,
      diagnostics: [
        DiagnosticSpec(
    ...
    assertIfConfig(
      "canImport(SwiftSyntaxOld) || canImport(ExplodingModule)",
      .active
    )
  }
