...
While SnapshotTestingOld comes with a wide variety of snapshot strategies, it can also be extended with
custom, user-defined strategies using the ``SnapshotTestingOld/Snapshotting`` and
``SnapshotTestingOld/Diffing`` types.

## Snapshotting

The ``SnapshotTestingOld/Snapshotting`` type represents the ability to transform a snapshottable value
...
``SnapshotTestingOld/Diffing`` strategy, you may need to create a base ``SnapshotTestingOld/Snapshotting``
value alongside it.

### Asynchronous Strategies

Some types need to be snapshot in an asynchronous fashion. ``SnapshotTestingOld/Snapshotting`` offers
...
The ``SnapshotTestingOld/Diffing`` type represents the ability to compare `Value`s and convert them to
and from `Data`.
